...
void Game::loadCommonSprites() {
	if (_isDemo) {
		...
		debugNetPrintf(DEBUG,"loadCommonSprites1");
		_bermudaOvrData = loadFile("ux0:data/SOMO00004/BERMUDA.OVR");
		debugNetPrintf(DEBUG,"loadCommonSprites2");
	}
	debugNetPrintf(DEBUG,"loadCommonSprites3");
	loadWGP("ux0:data/SOMO00004/BERMUDA.WGP");
	debugNetPrintf(DEBUG,"loadCommonSprites4");
...
debugNetPrintf(DEBUG,"loadCommonSprites5");
	loadFile("ux0:data/SOMO00004/BERMUDA.SPR", _tempDecodeBuffer);
	debugNetPrintf(DEBUG,"loadCommonSprites6");
	...
	debugNetPrintf(DEBUG,"loadCommonSprites7");
}
...
uint8_t *Game::loadFile(const char *fileName, uint8_t *dst, uint32_t *dstSize) {
	...
	snprintf(fileSystemPath, sizeof(fileSystemPath), "%s/%s", "ux0:data/SOMO00004", fileName);
	//File fp;
	fp->open(fileSystemPath, "rb"); 
...
void Game::loadWGP(const char *fileName) {
	...
	if(strncmp("..\\", fileName, strlen("..\\")) == 0){
		stringToUpperCase(fileName);
		debugNetPrintf(DEBUG,"loadWGP");
		char str[256];
		
		strcpy(str, "ux0:data/SOMO00004/");
		debugNetPrintf(DEBUG,"loadWGP %s",str);
		strcat(str, fileName+3);
		debugNetPrintf(DEBUG,"loadWGP %s",str);
		fileName=str;
		debugNetPrintf(DEBUG,"loadWGP %s",fileName);
	}
	if(strncmp("ux0", fileName, strlen("ux0")) < 0){
		char str[256];		
		strcpy(str, "ux0:data/SOMO00004/WGP/");
		strcat(str, fileName);
		fileName=str;
	}
...
void Game::loadSPR(const char *fileName, SceneAnimation *sa) {
	...
	if(strncmp("..\\", fileName, strlen("..\\")) == 0){
	stringToUpperCase(fileName);
		debugNetPrintf(DEBUG,"loadSPR");
		char str[256];
		
		strcpy(str, "ux0:data/SOMO00004/");
		debugNetPrintf(DEBUG,"loadSPR %s",str);
		strcat(str, fileName+3);
		debugNetPrintf(DEBUG,"loadSPR %s",str);
		fileName=str;
		debugNetPrintf(DEBUG,"loadSPR %s",fileName);
	}
	File* fp = new File;
	fp->open(fileName, "rb");
	int tag = fp->readUint16LE();
...
void Game::loadMOV(const char *fileName) {
	...
	stringToUpperCase(fileName);
		...
		strcpy(str, "ux0:data/SOMO00004/");
		debugNetPrintf(DEBUG,"loadMOV %s",str);
		strcat(str, fileName+3);
		debugNetPrintf(DEBUG,"loadMOV %s",str);
		fileName=str;
		debugNetPrintf(DEBUG,"loadMOV %s",fileName);
	}

...
void Game::parseSCN(const char *fileName) {
	...
	char str[256];
	strcpy(str, "ux0:data/SOMO00004/SCN/");
	strcat(str, fileName);
...
//	stringToUpperCase(fileName);
	...
	for (char *s = _sceneDescriptionBuffer; !_stopParsing && s; ) {
		...
		debugNetPrintf(DEBUG,"\n\nCurrent file %s \n\n", fileName);
			debugNetPrintf(DEBUG,"\n\nCurrent token %d state %d\n\n", _currentToken, _currentState);
			...
			case kParserTokenBox:
				...
				break;
			default:
			debugNetPrintf(DEBUG,"Unexpected token %d state %d", _currentToken, _currentState);
				//error("Unexpected token %d state %d", _currentToken, _currentState);
				//break;
			}
		}

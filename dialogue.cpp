...
void Game::initDialogue() {
	playMusic("ux0:data/SOMO00004/MIDI/SADIALOG.MID");
	...
	_dialogueFrameSpriteData = loadFile("ux0:data/SOMO00004/WGP/FRAME.SPR");
...
void Game::loadDialogueSprite(int spr) {
	...
	char *spriteFile = "ux0:data/SOMO00004/WGP/KISS.SPR";
	switch (spr) {
	case 0:
		spriteFile = _scriptDialogSprite1;
		if(strncmp("..\\", spriteFile, strlen("..\\")) == 0){
		stringToUpperCase(spriteFile);
			
			char str[256];
			
			strcpy(str, "ux0:data/SOMO00004/");
			
			strcat(str, spriteFile+3);

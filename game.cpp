...
_isDemo=false;
_startupScene="_02.SCN";
return;
	// underscore ('_') for french & english versions and dash ('-') for german
	static const char *startupScenesTable[] = { "ux0:data/SOMO00004/SCN/-01.SCN", "ux0:data/SOMO00004/SCN/_01.SCN", 0 };
	_startupScene = 0;
	for (int i = 0; startupScenesTable[i]; ++i) {
		File* fp = new File;
		fp->open(startupScenesTable[i], "rb");
		if (fp) {
	...
	_isDemo = false;
	File* fp = new File;
		fp->open("ux0:data/SOMO00004/SCN/-00.SCN", "rb");
		if (fp) {
			_isDemo =true;
		}
...
debugNetPrintf(DEBUG,"restart5");
	strcpy(_tempTextBuffer, _startupScene);
	debugNetPrintf(DEBUG,"restart4");
}
...
void Game::mainLoop() {
	...
	case kStateGame:
		while (_switchScene) {
			...
			debugNetPrintf(DEBUG,"mainLoop6");
			if (_loadDataState != 0) {
		...
		debugNetPrintf(DEBUG,"mainLoop90");
		runObjectsScript();
		debugNetPrintf(DEBUG,"mainLoop91");
		if (_startDialogue) {
			_startDialogue = false;
	...
	case kStateDialogue:
		...
		break;
	case kStateBitmapSequence:
		debugNetPrintf(DEBUG,"mainLoop42");
		if (_stub->_pi.enter) {
			...
			debugNetPrintf(DEBUG,"mainLoop45");
				restart();
				_nextState = kStateGame;
			} else {
				drawBitmapSequenceDemo(_bitmapSequence);
	...
	debugNetPrintf(DEBUG,"mainLoop222");
	_stub->updateScreen();
...
#endif
	_lastFrameTimeStamp = _stub->getTimeStamp();
	debugNetPrintf(DEBUG,"mainLoop22222");
}
...
void Game::runObjectsScript() {
	...
	if (_loadDataState == 2) {
		...
		for (int i = start; i < _sceneObjectsCount; ++i) {
			...
			while (_objectScript.dataOffset < endOfDataOffset) {
				...
				while (loop) {
					...
					debugNetPrintf(DEBUG, "\nstatement %d condition %d op %d", statement, i, op);
					...
					if (!cop) {
					debugNetPrintf(DEBUG,"Invalid condition  %d", op);
		...
		if (_varsTable[0] >= 10 && !_gameOver) {
			strcpy(_musicName, "ux0:data/SOMO00004/MIDI/GAMEOVER.MID");
			playMusic(_musicName);
			_gameOver = true;
...
void Game::drawBitmapSequenceDemo(int num) {
	static const char *bitmapList[] = { "ux0:data/SOMO00004/WGP/TITLE.BMP", "ux0:data/SOMO00004/WGP/TITLE1.BMP", "ux0:data/SOMO00004/WGP/TITLE2.BMP", "ux0:data/SOMO00004/WGP/TITLE3.BMP" };
...
void Game::playMusic(const char *name) {
	...
	if (name[0] == 0 || strncmp(name, "ux0:data/SOMO00004/MIDI/", 8) != 0) {
		return;
	}
